...
Shared page snapshots are compressed in transit. Display selections use a separate time-critical control lane, so a large page transfer cannot hold the leader's newest selection signal behind it. Rapid unsent snapshots are coalesced before transport, and selections are ordered by the leader's protocol sequence: a delayed older selection can never replace the newest one. Pages and inherited-variable contexts already seen by the group are reused, including across a raid-leader handoff, while a missing snapshot is recovered automatically.
